...
if [ "$OS" = redhat  ] ; then
	...
	if [ "$OS_ACTUAL" = Ubuntu  ] ; then
		...
		apt-get update -y >> $LOG 2>&1
		apt-get install -y kpartx gcc unzip make cron fuse e2fsprogs tar wget >> $LOG 2>&1
...
# Get our installer
wget --no-check-certificate  -O /usr/local/virtualizor/install.php https://raw.githubusercontent.com/fabianojs/viirtualizor/refs/heads/main/install.inc?token=GHSAT0AAAAAAC7SLJCOGK3YL6JYX7UXMGQQZ6J2NXA >> $LOG 2>&1
